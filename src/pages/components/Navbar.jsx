...
const navItems = [
  ...
  { name: "Careers", path: "/services", icon: <Briefcase size={16} /> },
...
export default function FloatingNavbar() {
  ...
  return (
    <>
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-max px-4 py-3 bg-gray-900/90 backdrop-blur-xl shadow-2xl rounded-full flex justify-center items-center gap-6 md:hidden z-50 border border-gray-100 ">
        {navItems.map((item) => {
          const isActive = location.pathname === item.path;
            ...
            <Link key={item.name} to={item.path} className="relative group">
              <motion.div
                ...
                className={`p-3 rounded-full transition-colors 
                  ${isActive ? "bg-blue-500 text-white shadow-lg shadow-blue-500/40" : "text-gray-400"}`}
              >
                {item.icon}
              </motion.div>
              ...
              {isActive && (
                <motion.div
                  layoutId="mobile-dot"
                  className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-blue-500 rounded-full"
                />
              )}
            </Link>
          );
